...
const CommentCard = ({ index, leftVal, commentData }) => {
    ...
    let { blog, blog: { comments, comments: { results: commentArr }, activity, activity: { total_parent_comments }, author: { personal_info: { username: blog_author } } }, setBlog, setTotalParentCommentsLoaded } = useContext(BlogContext);
    ...
    const loadReplies = ({ skip = 0, currentIndex = index }) => {
        if (commentArr[currentIndex].children.length) {
            ...
            axios.post(import.meta.env.VITE_SERVER_ROUTE + "/get-replies", { _id: commentArr[currentIndex]._id, skip })
                .then(({ data: { replies } }) => {
    ...
    const deleteComments = (e) => {
        ...
        axios.post(import.meta.env.VITE_SERVER_ROUTE + "/delete-comment", { _id }, {
            headers: {
