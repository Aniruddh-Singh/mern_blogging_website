...
const PublishForm = () => {
    ...
    const { blog, blog: { title, banner, content, tags, des } = { title: '', banner: '', content: [], tags: [], des: '' }, setEditorState, setBlog } = useContext(EditorContext);
    ...
    const publishBlog = (e) => {
        ...
        if (!title.length) {
            return toast.error("Write blog title before publishsing.")
        }

        if (!des.length || des.length > 200) {
            return toast.error(`Write description about your blog within ${charaterLimit} characters to publish.`)
        }
        ...
        <AnimationWrapper>
            <section className="w-screen min-h-screen grid items-center lg:grid-cols-2 py-16 lg:gap-4">
                ...
                <div className="max-w-[550px] center">
                    ...
                    <p className="font-gelasio line-clamp-2 text-xl leading-7 mt-4"></p>
                </div>
                ...
                <div className="border-grey lg:border-1 lg:pl-8 ">
                    ...
                    <textarea className="h-40 resize-none leading-7 input-box pl-4" maxLength={charaterLimit} defaultValue={des} onChange={handleBlogDesChange} onKeyDown={handleTitleKeyDown}>
                    </textarea>
                    <p className="mt-1 text-dark-grey text-sm text-right">{charaterLimit - des.length} characters left</p>
