...
const EditProfile = () => {
    ...
    const handleImgUpload = (e) => {
        ...
        if (updatedProfileImg) {
            ...
            uploadImage(updatedProfileImg)
                .then(url => {
                    if (url) {
                        axios.post(import.meta.env.VITE_SERVER_ROUTE + "update-profile-img", { url }, {
                            headers: {
    ...
    const handleSubmit = (e) => {
        ...
        let { username, bio, youtube, facebook, instagram, twitter, github, website } = formData;
        ...
        axios.post(import.meta.env.VITE_SERVER_ROUTE + "/update-profile", { username, bio, social_links: { youtube, facebook, twitter, github, instagram, website } }, {
            headers: {
                'Authorization': `Bearer ${accessToken}`
            }
        })
    ...
    useEffect(() => {
        if (accessToken) {
            axios.post(import.meta.env.VITE_SERVER_ROUTE + "/get-profile", { username: userAuth.username })
                .then(({ data }) => {
                    setProfile(data);
                    setLoading(false);
                })
